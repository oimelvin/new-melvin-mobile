...
const ModalCaracteristicas: React.FC<ModalCaracteristicasProps> = ({
  ...
  >(null)
	...
	const { getCaracteristicasAtivoById } = useAtivoService()
		...
		const loadCaracteristicas = async () => {
			try {
				...
				if (idSelecionado && tipo) {
					...
					switch (tipo) {
					case 'A':
						caracteristicas = await getCaracteristicasAtivoById(idSelecionado)
						break
					case 'C':
						caracteristicas = await getCaracteristicasConjuntoEquipamentoById(
							idSelecionado
						)
						break
					case 'C':
			...
			>
				<View style={{ flexDirection: 'row' }}>
					<View style={{ flex: 1 }}>
						...
						{subTituloModal && (
							<>
								<Text style={{ fontWeight: 'bold' }}>{subTituloModal}</Text>
				...
				{loading ? (
					...
					<FlatList<Caracteristica>
						...
						ListEmptyComponent={<Text>Nenhuma característica encontrada.</Text>}
						renderItem={({ item }) => (
							<View key={item.id}>
								<Field
